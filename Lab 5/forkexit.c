...
/* The process pointed to by 'mp' has forked.  Create a child process. */
  ...
  *rmc = *rmp;			/* copy parent's process slot to child's */
  ...
  // Print that process was created and mention PID
  printf("Minix (200010036/37): PID %d created (Endpoint: %d)\n", rmc->mp_pid, _ENDPOINT_P(rmc->mp_endpoint));
...
int do_exit()
{
  ...
  */
  ...
  // Print that process exited and mention PID
  printf("Minix (200010036/37): PID %d exited (Endpoint %d)\n", mp->mp_pid, _ENDPOINT_P(mp->mp_endpoint));
