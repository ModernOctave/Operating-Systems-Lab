...
int main(int argc, char const *argv[])
{
	...
	while (!sjf.ready.empty() || !sjf.waiting.empty())
	{
		context_switches++;
		time = sjf.run(time);
	}
	...
	printf("Average\t%f\t%f\t%f\n", (float)total_waiting_time / sjf.finished.size(), (float)total_turnaround_time / sjf.finished.size(), total_penalty_ratio / sjf.finished.size());
	printf("Throughput: %f\n", (float)sjf.finished.size() / (time+context_switches*context_switch_overhead));
	return 0;
}
