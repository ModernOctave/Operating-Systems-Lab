...
int fs_readwrite(void)
{
  ...
  // If writing to the immediate file exceeds the 32 byte limit convert it to a regular file
  if (mode_word == I_IMMEDIATE && rw_flag == WRITING && (position + nrbytes) > 32) {
    char buffer[32];
    register struct buf* bp;

    // Copy the data to a buffer
    for (int i = 0; i < f_size; i++) {
      buffer[i] = rip->i_zone[i];
    }
  ...
  // Handle immediate file read/write
